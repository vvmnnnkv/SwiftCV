...
public enum VideoCaptureProperties: Int32 {
    case CAP_PROP_POS_MSEC = 0 //!< Current position of the video file in milliseconds.
    case CAP_PROP_POS_FRAMES = 1 //!< 0-based index of the frame to be decoded/captured next.
    case CAP_PROP_POS_AVI_RATIO = 2 //!< Relative position of the video file: 0=start of the film, 1=end of the film.
    case CAP_PROP_FRAME_WIDTH = 3 //!< Width of the frames in the video stream.
    case CAP_PROP_FRAME_HEIGHT = 4 //!< Height of the frames in the video stream.
    case CAP_PROP_FPS = 5 //!< Frame rate.
    case CAP_PROP_FOURCC = 6 //!< 4-character code of codec. see VideoWriter::fourcc .
    case CAP_PROP_FRAME_COUNT = 7 //!< Number of frames in the video file.
    case CAP_PROP_FORMAT = 8 //!< Format of the %Mat objects (see Mat::type()) returned by VideoCapture::retrieve().

    case CAP_PROP_MODE = 9 //!< Backend-specific value indicating the current capture mode.
    case CAP_PROP_BRIGHTNESS = 10 //!< Brightness of the image (only for those cameras that support).
    case CAP_PROP_CONTRAST = 11 //!< Contrast of the image (only for cameras).
    case CAP_PROP_SATURATION = 12 //!< Saturation of the image (only for cameras).
    case CAP_PROP_HUE = 13 //!< Hue of the image (only for cameras).
    case CAP_PROP_GAIN = 14 //!< Gain of the image (only for those cameras that support).
    case CAP_PROP_EXPOSURE = 15 //!< Exposure (only for those cameras that support).
    case CAP_PROP_CONVERT_RGB = 16 //!< Boolean flags indicating whether images should be converted to RGB.
    case CAP_PROP_WHITE_BALANCE_BLUE_U = 17 //!< Currently unsupported.
    case CAP_PROP_RECTIFICATION = 18 //!< Rectification flag for stereo cameras (note: only supported by DC1394 v 2.x backend currently).
    case CAP_PROP_MONOCHROME = 19
    case CAP_PROP_SHARPNESS = 20
    case CAP_PROP_AUTO_EXPOSURE = 21 //!< DC1394: exposure control done by camera, user can adjust reference level using this feature.
    case CAP_PROP_GAMMA = 22
    case CAP_PROP_TEMPERATURE = 23
    case CAP_PROP_TRIGGER = 24
    case CAP_PROP_TRIGGER_DELAY = 25
    case CAP_PROP_WHITE_BALANCE_RED_V = 26
    case CAP_PROP_ZOOM = 27
    case CAP_PROP_FOCUS = 28
    case CAP_PROP_GUID = 29
    case CAP_PROP_ISO_SPEED = 30
    case CAP_PROP_BACKLIGHT = 32
    case CAP_PROP_PAN = 33
    case CAP_PROP_TILT = 34
    case CAP_PROP_ROLL = 35
    case CAP_PROP_IRIS = 36
    case CAP_PROP_SETTINGS = 37 //!< Pop up video/camera filter dialog (note: only supported by DSHOW backend currently. The property value is ignored)
    case CAP_PROP_BUFFERSIZE = 38
    case CAP_PROP_AUTOFOCUS = 39
    case CAP_PROP_SAR_NUM = 40 //!< Sample aspect ratio: num/den (num)
    case CAP_PROP_SAR_DEN = 41 //!< Sample aspect ratio: num/den (den)
    case CAP_PROP_BACKEND = 42 //!< Current backend (enum VideoCaptureAPIs). Read-only property
    case CAP_PROP_CHANNEL = 43 //!< Video input or Channel Number (only for those cameras that support)
    case CAP_PROP_AUTO_WB = 44 //!< enable/ disable auto white-balance
    case CAP_PROP_WB_TEMPERATURE = 45 //!< white-balance color temperature
    case CAP_PROP_CODEC_PIXEL_FORMAT = 46 //!< (read-only) codec's pixel format. 4-character code - see VideoWriter::fourcc . Subset of [AV_PIX_FMT_*](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/raw.c) or -1 if unknown
}
